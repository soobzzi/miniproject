...
public class UserDAO {
	...
	public HashMap<String,Object> getUserList(SearchVO searchVO) throws Exception {
		...
		// YPE_SCROLL_INSENSITIVE 으로 옵션만들면 rs.next() 이후에 앞으로 못돌아오던거를 돌아오게가능 rs.last()를 하기
		//위해선 필요함 ResultSet.CONCUR_UPDATABLE : 커서의 위치에서 정보 업데이트 가능. ~row() 변경메소드일때 사용
		ResultSet rs = stmt.executeQuery();
		...
		//pageUnit은 페이지당 항목수 ,page는 현재 페이지 번호
