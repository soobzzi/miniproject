				...
				$.ajax( 
						{
							...
							data : JSON.stringify({//RequestBody에 바인딩해야하는데 객체가 쿼리스트링으로 들어오니까 이걸 스트링화(text로!)해서 rawtext로 만드는거쥥 그럼 바인딩됨  
													// modelAttribute를 써도되긴하는데 쓰면 stringify 없어도됨! 하지만 약속으로 body쓰자고~
								userId : id,
								password : pw
							}),//JSONObject -> 쿼리스트링으로변환
							success : function(JSONData , status) {
								...
								if( JSONData != null ){ //null이 들어오면? ->만들기
									//[방법1]
									//$(window.parent.document.location).attr("href","/index.jsp");//ajax필요없음
									
									//[방법2]
									//window.parent.document.location.reload(); //reload가뭐지
